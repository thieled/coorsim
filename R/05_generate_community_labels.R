...
label_communities <- function(groups_data,
  ...
  col_order <- c(
    "community", "label", "description", "lang", "n_accs", "n_posts", "sum_user_share_comm",
    "is_valid_json", "response", "text", "model", "model_queried_at", "model_total_duration"
  )
  
  res_dt <- res_dt[, intersect(col_order, names(res_dt)), with = FALSE]
