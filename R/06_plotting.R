...
#' @param use_palette Optional custom palette function(n) or a character vector
#'   of hex colors (length >= number of communities).
#' @param use_order Optional vector specifying the order in which communities
...
plot_posts <- function(network_data,
  ...
  get_cap <- function(label) {
    ...
    vals <- mapply(
      function(nm, v) {
        paste(
          nm,
          if (is.numeric(v)) paste(round(v, 2), collapse = ", ") else paste(v, collapse = ", "),
          sep = ": "
        )
      },
      nm = names(x),
  ...
  title <- if (!is.null(title_prefix)) {
    paste(title_prefix, "Coordinated posts over time")
  } else {
    "Coordinated posts over time"
  }
  
  # Communities: character for mapping, numeric order if possible
  dt[, community := as.character(community)]
  unique_comms <- unique(dt$community)
  tmp_num <- suppressWarnings(as.numeric(unique_comms))
  if (!all(is.na(tmp_num))) {
    unique_comms <- unique_comms[order(tmp_num)]
  } else {
    unique_comms <- sort(unique_comms)
  }
  
  # Apply user-defined order
  if (!is.null(use_order)) {
    use_order <- as.character(use_order)
    intersection <- intersect(use_order, unique_comms)
    remaining <- setdiff(unique_comms, intersection)
    unique_comms <- c(intersection, remaining)
  }
  ...
  if ("community_labels" %in% names(network_data)) {
    ...
    agg[!is.na(label),
        community_label := paste0("No. ", community, ": ", label, " [n_posts=", n_comm_posts, "]")]
  }
  ...
  # Build full grid (community × time) and fill missing with N = 0
  all_times <- sort(unique(agg$time_floored))
  grid <- data.table::CJ(community = unique_comms, time_floored = all_times, unique = TRUE)
  agg_full <- merge(grid, agg, by = c("community", "time_floored"), all.x = TRUE)
  agg_full[is.na(N), N := 0L]
  
  if (!by_community) {
    
    # Combined plot uses full grid
    p <- ggplot2::ggplot(
      agg_full,
      ggplot2::aes(x = time_floored, y = N, color = community, group = community)
    ) +
...
#' @param palette_option Character. Viridis palette option (e.g., "A", "B", "C", "D").
#' @param end_color Numeric. End value for viridis color scale (between 0 and 1).
#' @param start_color Numeric. Start value for viridis color scale (between 0 and 1).
#' @param title_prefix Optional prefix for the plot title.
#' @param n_top_nodes Integer. Number of top-degree nodes per community to label.
#' @param label_fontsize Integer. Base font size for text panels. If NULL, size is auto-scaled.
#' @param by_community Logical. If TRUE, create individual plots per community with metadata side panels. 
#' @param ncol Integer. Number of columns in the grid layout when by_community = TRUE.
...
plot_communities <- function(network_data, 
  ...
  required_pkgs <- c(
    ...
    "ggforce", "tidygraph", "ggtext", "scales", "viridis", "data.table", "stringr"
  )
  for (pkg in required_pkgs) {
    if (!requireNamespace(pkg, quietly = TRUE)) {
      stop(
        paste0(
          "Package '", pkg,
          "' is required for this function. Please install it using install.packages(\"",
          pkg, "\")."
        ),
        call. = FALSE
      )
  ...
  if (!is.null(network_data$params$cluster_method) &&
      network_data$params$cluster_method != "FSA_V") {
    network_data$params$theta <- NULL
  }
  
  # Communities (numeric sort if possible)
  unique_communities <- unique(dplyr::filter(node_list, account_id %in% igraph::V(g)$name)$community)
  unique_communities <- as.character(unique_communities)
  tmp_num <- suppressWarnings(as.numeric(unique_communities))
  if (!all(is.na(tmp_num))) {
    unique_communities <- unique_communities[order(tmp_num)]
  } else {
    unique_communities <- sort(unique_communities)
  }
  
  # Apply user-defined order
  if (!is.null(use_order)) {
    use_order <- as.character(use_order)
    intersection <- intersect(use_order, unique_communities)
    remaining <- setdiff(unique_communities, intersection)
    unique_communities <- c(intersection, remaining)
  }
  ...
  if (is.null(label_fontsize)) {
    label_fontsize <- round(max(6, 12 - length(unique_communities) / 12 - (ncol - 1) * 0.5), 0)
  }
  ...
  if (!by_community) {
    ...
    p <- ggraph::ggraph(layout) +
      ...
      ggplot2::labs(
        title = title,
        caption = paste0(stringr::str_wrap(param_string, width = 200, whitespace_only = TRUE),
                         "\n", stringr::str_wrap(filter_string, width = 200, whitespace_only = TRUE)),
        color = "Community")
    ...
    for (c in unique_communities) {
      ...
      meta <- comm_df |> dplyr::filter(community == c)
